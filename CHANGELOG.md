...
- **BSD man page layout support** — options listed inside `DESCRIPTION` after "The following options are available:" / "The options are as follows:" are parsed, in addition to the GNU `OPTIONS` section. Option indentation is inferred from the first option line instead of being hardcoded to GNU's 7 columns (BSD uses 5).
